//! Remote to local transfer executor
//!
//! Streams remote SFTP files to local destination.
...
pub struct RemoteToLocalExecutor {
    source: Arc<dyn FileSource>,
    dest: Arc<dyn FileSource>,
    config: TransferConfig,
}

impl RemoteToLocalExecutor {
    pub fn new(
        source: Arc<dyn FileSource>,
        dest: Arc<dyn FileSource>,
        config: TransferConfig,
...
#[async_trait]
impl TransferExecutor for RemoteToLocalExecutor {
    async fn execute(
        &self,
        ...
        for (index, file) in files.files.into_iter().enumerate() {
            ...
            // fall back to SFTP when the server does not allow exec. On the
            // stream, end-of-stream and a dropped connection look identical,
            // so the byte count is verified after the copy loop.
